...
def request_data():
    ...
    elif len(get_session.cookies) == 1 or len(get_session_cookies) > 1:
    ...
    for i in items:
        ...
        if dt_date in week_schedules.keys():
            if dt_hr in week_schedules[dt_date]:
                print("Sorty on: ", dt_date, " ", dt_time, " was changed/updated: ")
                continue
            elif dt_hr not in week_schedules[dt_date]:
                latest[dt_hr] = {"TIME": dt_hr , "CAPITAN": i[1], "CREW": i[2], "AIRCRAFT": i[3], "MODULE": i[4], "EXCERCISE": i[5], "DESCRIPTION": i[6], "FLY_TYPE": i[7]}
                existing = {}
                ...
                week_schedules[dt_date] = {dt_hr:{"TIME": dt_hr, "CAPITAN": i[1], "CREW": i[2], "AIRCRAFT": i[3], "MODULE": i[4], "EXCERCISE": i[5], "DESCRIPTION": i[6], "FLY_TYPE": i[7]}}   
...
def cli_display(schedules):
    ...
    table.set_cols_width([10,10,10,10, 10, 6, 10, 12, 10])
    table.header(["DATE", "TIME", "CAPITAN", "CREW", "AIRCRAFT", "MODULE", "EXCERCISE", "DESCRIPTION", "FLY-TYPE"])
